/*!
 * gulp
 * $ OLD DO NOT USE npm install gulp-ruby-sass gulp-autoprefixer gulp-minify-css gulp-jshint gulp-concat gulp-uglify gulp-imagemin gulp-notify gulp-rename gulp-livereload gulp-cache del --save-dev
 * $ npm install gulp gulp-sass gulp-scss-lint gulp-jshint gulp-rename gulp-plumber gulp-livereload gulp-ftp gulp-sourcemaps del vinyl-source-stream browserify vinyl-buffer gulp-uglify --save-dev
 */
...
gulp.task('scripts_production', ['js_lint'], function() {
    ...
    var b = browserify({debug: false, cache: {}, packageCache: {}, fullPaths: true });
    b.add('./src/js/main.js');
    return b.bundle()
        .on('error', function(e){ console.log(e); this.emit('end'); })
        .pipe(source('scripts.min.js')) // output file name
